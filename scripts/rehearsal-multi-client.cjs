...
Options:
  ...
  --room-name <name>    Room name the host creates (default "${DEFAULT_ROOM_NAME}")
...
async function createRoom(page, roomName) {
  ...
  await page.waitForURL(/\/room\?code=[A-F0-9]{8}/, { timeout: WAIT_TIMEOUT_MS });
...
async function joinRoomByInviteCode(page, baseUrl, code, name) {
  await page.goto(`${baseUrl}/room?code=${code}`);
...
async function leaveWaitingRoom(page) {
  const link = page.locator(".waiting-room").getByRole("link", { name: "처음으로" });
  await link.waitFor({ timeout: WAIT_TIMEOUT_MS });
  await link.click({ timeout: WAIT_TIMEOUT_MS });
  await page.waitForURL(/\/join$/, { timeout: WAIT_TIMEOUT_MS });
  return page.url();
}
...
async function runJourney({ mode, baseUrl, roomName, runs, headless }) {
  ...
  try {
    ...
    for (let run = 1; run <= runs; run += 1) {
      await evidence.step(
        ...
        async () => {
          ...
          await host.waitForTimeout(SHORT_SETTLE_MS);
          const tallies = await Promise.all([host, guestA, guestB].map((page) => page.locator(".say-balance__count").allInnerTexts()));
          assert.deepEqual(tallies[0], tallies[1]);
          assert.deepEqual(tallies[1], tallies[2]);
          return { tallies: tallies[0] };
        },
      ...
      await evidence.step(
        ...
        "host",
        async () => {
          const before = await host.locator(".say-balance__count").allInnerTexts();
          await host.locator(".say-balance__choices button").nth(1).click();
          await host.waitForTimeout(SHORT_SETTLE_MS);
          const after = await host.locator(".say-balance__count").allInnerTexts();
          const totalBefore = before.reduce((sum, text) => sum + Number.parseInt(text, 10), 0);
          const totalAfter = after.reduce((sum, text) => sum + Number.parseInt(text, 10), 0);
    ...
    await evidence.step(
      ...
      "host",
      async () => {
        await host.goto(`${baseUrl}/room?code=${inviteCode}`);
    ...
    await evidence.step(
      ...
      "guestB",
      async () => {
        const url = await leaveWaitingRoom(guestB);
    ...
    for (let run = 1; run <= runs; run += 1) {
      await evidence.step(
        ...
        async () => {
          ...
          await Promise.all([
            host.locator("#room-chat-message").fill(markers[0]).then(() => host.getByRole("button", { name: "보내기", exact: true }).click()),
            guestA.locator("#room-chat-message").fill(markers[1]).then(() => guestA.getByRole("button", { name: "보내기", exact: true }).click()),
          ]);
          await host.waitForTimeout(SHORT_SETTLE_MS);
