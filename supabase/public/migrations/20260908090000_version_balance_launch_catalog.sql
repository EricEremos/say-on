-- Keep existing rooms on their original catalog; new clients select the versioned launch set.
...
alter table public.group_sessions
  ...
  check (selected_game_key in ('icebreaker', 'balance', 'balance-ko-2026-09'));
...
begin
  ...
  if p_game_key is null or p_game_key not in ('icebreaker', 'balance-ko-2026-09') then
...
begin
  ...
  question_count := case current_session.selected_game_key when 'balance' then 30 when 'balance-ko-2026-09' then 20 else 17 end;
...
begin
  ...
  if current_session.selected_game_key in ('balance', 'balance-ko-2026-09') and current_session.phase = 'live' then
...
begin
  ...
  if not found or current_session.phase <> 'live' or current_session.selected_game_key is null or current_session.selected_game_key not in ('balance', 'balance-ko-2026-09') then
